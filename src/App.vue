...
function checkOverflow() {
  ...
  // Measure nav width once (it doesn't change — same links always)
  if (cachedNavWidth === 0) {
    ...
    nav.style.display = ''
    nav.style.flex = ''
    if (collapsed.value) nav.classList.add('nav-hidden')
  }

  const headerWidth = headerRef.value.offsetWidth
  // Use fixed estimates for left (logo 50 + gap + text ~170) and right (icons ~90) + padding
  // This avoids the feedback loop where collapsed state changes left width
  const right = headerRef.value.querySelector('.header-actions') as HTMLElement | null
  const fixedLeftWidth = 262 // logo(50) + gap(8) + "Montajate Financier" text (~204)
  const rightWidth = right?.offsetWidth ?? 90
  const totalNeeded = cachedNavWidth + fixedLeftWidth + rightWidth + 24 // 24 for header gaps

  const shouldCollapse = totalNeeded > headerWidth
  ...
  <PinGate>
    <div class="app-shell">
      <header ref="headerRef" class="app-header">
        ...
        <nav ref="navRef" class="desktop-nav" :class="{ 'nav-hidden': collapsed }">
          <!-- ...existing nav badges... -->
      ...
      <Transition name="fade">
        <div v-if="menuOpen" class="sidebar-backdrop" @click="closeMenu"></div>
      </Transition>
      <Transition name="slide-left">
        <nav v-if="menuOpen" class="mobile-sidebar">
          ...
          <RouterLink to="/budgets" class="sidebar-link" @click="closeMenu">
            <font-awesome-icon :icon="['fas', 'wallet']" /> Budgets
          </RouterLink>
...
/* ─── Small screens: shrink logo, hide title ─── */
