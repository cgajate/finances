  ...
  <div class="page">
    ...
    <div class="summary-cards">
      <div class="card earned">
        ...
        <span class="card-value">{{ formatCurrency(review.totalIncome) }}</span>
      </div>
      <div class="card spent">
        <span class="card-label">Total Spent</span>
        <span class="card-value">{{ formatCurrency(review.totalExpenses) }}</span>
      </div>
      <div class="card net" :class="{ negative: review.netSavings < 0 }">
        <span class="card-label">Net Savings</span>
        <span class="card-value">{{ formatCurrency(review.netSavings) }}</span>
      </div>
      <div class="card rate" :class="{ negative: review.savingsRate < 0 }">
        <span class="card-label">Savings Rate</span>
        <span class="card-value">{{ review.savingsRate }}%</span>
      </div>
    </div>
    ...
    <div class="month-chart">
      <div v-for="m in review.months" :key="m.month" class="month-col">
        ...
        <div class="month-net" :class="{ negative: m.net < 0 }">
...
.card { padding: 1rem; border-radius: 12px; display: flex; flex-direction: column; gap: 0.3rem; }
.card-label { font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.04em; opacity: 0.8; }
.card-value { font-size: 1.3rem; font-weight: 700; }
.card.earned { background: var(--color-income-bg); color: var(--color-income); }
.card.spent { background: var(--color-expense-bg); color: var(--color-expense); }
.card.net { background: var(--color-primary-light); color: var(--color-primary-text); }
.card.net.negative { background: var(--color-warning-bg); color: var(--color-warning); }
.card.rate { background: var(--color-assigned-bg); color: var(--color-assigned-text); }
.card.rate.negative { background: var(--color-warning-bg); color: var(--color-warning); }
...
.month-col { display: flex; flex-direction: column; align-items: center; gap: 0.25rem; min-width: 48px; flex: 1; }
.month-bars { display: flex; gap: 2px; align-items: flex-end; height: 80px; }
.month-bar { width: 16px; border-radius: 3px 3px 0 0; min-height: 2px; }
.income-bar { background: var(--color-progress-fill); }
.expense-bar { background: var(--color-btn-delete); }
.month-label { font-size: 0.7rem; font-weight: 600; color: var(--color-text-muted); }
.month-net { font-size: 0.6rem; font-weight: 600; color: var(--color-income); white-space: nowrap; }
.month-net.negative { color: var(--color-expense); }
